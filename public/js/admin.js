...
const initializeSocketConnection = (participant) => {
    ...
    socket.on("auctionStarted", (response) => {
        response = JSON.parse(response);
        addMessage(response.message);
        startAuctionTimer(response.payload.startTime);
    });
    ...
    socket.on("pictureAuctionFinished", (response) => {
        ...
        if (response.payload.sold) {
            $(`.picture_card[data-id="${response.payload.painting['id']}"]`).append($(`
    ...
    socket.on("changePrice", (response) => {
        response = JSON.parse(response);
        $(`.current_picture_price[data-id="${response.payload.painting['id']}"]`).text(response.payload.newPrice);
    });
    socket.on("changeParticipantCash", (response) => {
        response = JSON.parse(response);
        $(`.participant_cache[data-id="${response.payload.pID}"]`).text(response.payload.cashReserve);
    });
};
